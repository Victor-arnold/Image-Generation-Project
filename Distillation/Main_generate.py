...
import torch
import os
from train_utils import make_visualization,generate_images
import cv2
import time
...
def make_argument_parser():
    ...
    parser.add_argument("--clipping_value", help="Noise clipping value.", type=float, default=1)
...
def sample_images(args, make_model):
    ...
    def make_diffusion(args, model, n_timestep, time_scale, device):
        ...
        D = getattr(M, args.diffusion) # GaussianDiffusion
        sampler = "ddpm"
        # 这个clip暂时不会使用
    ...
    # 生成图片并可视化 函数在train_utils.py里面
    start_time = time.time()
    for i in range(args.num):
        img = generate_images(teacher_diffusion, device, image_size, need_tqdm=True, eta=args.eta,
        ...
        # 保存图片
        cv2.imwrite(args.out_file+str(i)+".png", img)
