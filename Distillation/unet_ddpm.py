...
class ResBlock(nn.Module):
    def __init__(self, in_channel, out_channel, time_dim, use_affine_time=False, dropout=0, group_norm=32, ):
        super().__init__()
...
class ResBlockWithAttention(nn.Module):
    def __init__(self, in_channel, out_channel, time_dim, dropout, use_attention=False, attention_head=1,
                 use_affine_time=False, group_norm=32, ):
        super().__init__()
...
def spatial_fold(input, fold):
    ...
    return (input.view(batch, channel, h_fold, fold, w_fold, fold).permute(0, 1, 3, 5, 2, 4).reshape(batch, -1, h_fold,
                                                                                                     w_fold))
...
def spatial_unfold(input, unfold):
    ...
    return (input.view(batch, -1, unfold, unfold, height, width).permute(0, 1, 4, 2, 5, 3).reshape(batch, -1, h_unfold,
                                                                                                   w_unfold))
...
class UNet(nn.Module):
    def __init__(
        ...
        self.time = nn.Sequential(TimeEmbedding(channel), linear(channel, time_dim), Swish(),
