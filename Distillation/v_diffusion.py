...
# 创建diffusion  这个函数是方便外部调用
def make_diffusion(model, n_timestep, time_scale, device):
    betas = make_beta_schedule("cosine", n_timestep=n_timestep, cosine_s=8e-3).to(device)
...
class GaussianDiffusion:
    ...
    def __init__(self, net, betas, time_scale=1, sampler="ddpm"):
        ...
        betas = betas.type(torch.float64)  # 获取每次扩散的方差值
        self.num_timesteps = int(betas.shape[0])  # 获取时间戳步数

        # 这个部分是根据betas值确定一些固定的参数
        alphas = 1 - betas
