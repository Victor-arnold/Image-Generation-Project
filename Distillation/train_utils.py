...
def p_sample_loop(diffusion, noise, extra_args, device, eta=0, samples_to_capture=-1, need_tqdm=True, clip_value=3):
    ...
    if need_tqdm:
        iter_ = tqdm(iter_)
    for i in iter_:
        img = diffusion.p_sample(img, torch.full((img.shape[0],), i, dtype=torch.int64).to(device), extra_args, eta=eta,
...
def generate_images_(diffusion, device, image_size, need_tqdm=False, eta=0, clip_value=1.2):
    extra_args = {}
    noise = torch.randn(image_size, device=device)
    imgs = p_sample_loop(diffusion, noise, extra_args, "cuda", samples_to_capture=5, need_tqdm=need_tqdm, eta=eta,
                         clip_value=clip_value)
    
    return imgs[-1]
    

def generate_images(diffusion, device, image_size, need_tqdm=False, eta=0, clip_value=1.2):
    images_ = generate_images_(diffusion, device, image_size, need_tqdm=need_tqdm, eta=eta, clip_value=clip_value)
...
def make_iter_callback(diffusion, device, checkpoint_path, image_size, tensorboard, log_interval, ckpt_interval,
                       need_tqdm=False):
    state = {
        "initialized": False,
        "last_log": None,
    ...
    def iter_callback(N, loss, last=False):
        ...
        if ((t - state["last_ckpt"]).total_seconds() / 60 > ckpt_interval) or last:
            torch.save({"G": diffusion.net_.state_dict(), "n_timesteps": diffusion.num_timesteps,
                        "time_scale": diffusion.time_scale}, os.path.join(checkpoint_path, f"checkpoint.pt"))
            print("Saved.")
            state["last_ckpt"] = t
...
class DiffusionTrain:
    ...
    def train(self, train_loader, diffusion, model_ema, model_lr, device, make_extra_args=make_none_args,
...
class DiffusionDistillation:
    ...
    def train_student_debug(self, distill_train_loader, teacher_diffusion, student_diffusion, student_ema, student_lr,
                            device, make_extra_args=make_none_args, on_iter=default_iter_callback):
    ...
    def train_student(self, distill_train_loader, teacher_diffusion, student_diffusion, student_ema, student_lr, device,
                      make_extra_args=make_none_args, on_iter=default_iter_callback):
        scheduler = self.scheduler
        total_steps = len(distill_train_loader)
        scheduler.init(student_diffusion, student_lr, total_steps)
        teacher_diffusion.net_.eval()
